 ...
 */
  ...
  $args = array();
  $args['plugin'] = null;
  $args['controller'] = 'co_normalizers';
  $args['action'] = 'index';
  $args['co'] = $cur_co['Co']['id'];
  $this->Html->addCrumb(_txt('ct.co_normalizers.pl'), $args);
  $crumbTxt = _txt('op.reorder-a', array(_txt('ct.co_normalizers.pl')));
  ...
  $(function() {
    ...
    $("#sortable").sortable({
      update: function( event, ui ) {
        ...
        var jqxhr = $.post("<?php print $this->Html->url(array('controller' => 'co_normalizers',
                                                               'action'     => 'reorder',
                                                               'ext'        => 'json',
  ...
  <table id="co_navigation_links">
    <thead>
      <tr>
        ...
        <th><?php print _txt('fd.description'); ?></th>
    ...
    <tbody id="sortable">
      ...
      <?php foreach ($co_normalizers as $c): ?>
        <tr id = "CoNormalizerId_<?php print $c['CoNormalizer']['id']?>" class="line1">
          <td class = "order">
          ...
          <td>
            <?php
              print $this->Html->link($c['CoNormalizer']['description'],
                                      array('controller' => 'co_normalizers',
                                            'action' => ($permissions['edit'] ? 'edit' : ($permissions['view'] ? 'view' : '')), $c['CoNormalizer']['id']));
            ?>
          </td>
          <td><?php print $c['CoNormalizer']['plugin']; ?></td>
        </tr>
      <?php $i++; ?>
      <?php endforeach; ?>
    </tbody>
