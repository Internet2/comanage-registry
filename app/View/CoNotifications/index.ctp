 ...
 */
  ...
  if(!empty($this->request->query['status'])) {
    $curstatus = filter_var($this->request->query['status'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
  }
  ...
  if(!empty($this->request->query['sort'])) {
    $sorttype = filter_var($this->request->query['sort'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
  }
  
  if(!empty($this->request->query['direction'])) {
    $sortdir = filter_var($this->request->query['direction'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
  }
...
<table id="co_notifications" class="ui-widget">
  ...
  <tbody>
    ...
    <tr class="line<?php print ($i % 2)+1; ?>">
      <td><?php print filter_var($c['CoNotification']['action'],FILTER_SANITIZE_FULL_SPECIAL_CHARS); ?></td>
      <td><?php print $this->Html->link(filter_var($c['CoNotification']['comment'],FILTER_SANITIZE_FULL_SPECIAL_CHARS),
                                        array(
