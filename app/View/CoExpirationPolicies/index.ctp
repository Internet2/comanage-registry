...
<?php
  ...
  if($permissions['add']) {
    $sidebarButtons[] = array(
      'icon'    => 'circle-plus',
      'title'   => _txt('op.add-a',array(_txt('ct.co_expiration_policies.1'))),
      'url'     => array(
      ...
      <td>
        <?php
          ...
          if($permissions['delete'])
            print '<button class="deletebutton" title="' . _txt('op.delete') . '" onclick="javascript:js_confirm_delete(\'' . _jtxt(Sanitize::html($c['CoExpirationPolicy']['description'])) . '\', \'' . $this->Html->url(array('controller' => 'co_expiration_policies', 'action' => 'delete', $c['CoExpirationPolicy']['id'])) . '\')";>' . _txt('op.delete') . '</button>';
        ?>
        <?php ; ?>
      </td>
    </tr>
