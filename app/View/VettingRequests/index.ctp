<?php
/**
 * COmanage Registry Vetting Requests Index View
 *
 * Portions licensed to the University Corporation for Advanced Internet
 * Development, Inc. ("UCAID") under one or more contributor license agreements.
 * See the NOTICE file distributed with this work for additional information
 * regarding copyright ownership.
 *
 * UCAID licenses this file to you under the Apache License, Version 2.0
 * (the "License"); you may not use this file except in compliance with the
 * License. You may obtain a copy of the License at:
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 * 
 * @link          https://www.internet2.edu/comanage COmanage Project
 * @package       registry
 * @since         COmanage Registry v4.1.0
 * @license       Apache License, Version 2.0 (http://www.apache.org/licenses/LICENSE-2.0)
 */

  // Add breadcrumbs
  print $this->element("coCrumb");
  
  if(!empty($this->request->params['named']['copersonid'])) {
    // CO Person History
    $args = array();
    $args['plugin'] = null;
    $args['controller'] = 'co_people';
    $args['action'] = 'index';
    $args['co'] = $cur_co['Co']['id'];
    $this->Html->addCrumb(_txt('me.population'), $args);
    
    $args = array(
      'controller' => 'co_people',
      'action' => 'canvas',
      filter_var($this->request->params['named']['copersonid'],FILTER_SANITIZE_SPECIAL_CHARS)
    );
    if(isset($display_name)) {
      $this->Html->addCrumb($display_name, $args);
    } else {
      $this->Html->addCrumb(_txt('ct.co_people.1'), $args);
    }
  }
  
  $this->Html->addCrumb(_txt('ct.vetting_requests.pl'));

  // Add page title
  $params = array();
  $params['title'] = $title_for_layout;

  // Add top links
  $params['topLinks'] = array();

  if($permissions['register'] && !empty($this->request->params['named']['copersonid'])) {
    $params['topLinks'][] = $this->Html->link(
      _txt('op.register-a', array(_txt('ct.vetting_requests.1'))),
      array(
        'controller' => 'vetting_requests',
        'action' => 'register',
        'copersonid' => filter_var($this->request->params['named']['copersonid'],FILTER_SANITIZE_SPECIAL_CHARS)
      ),
      array('class' => 'addbutton')
    );
  }
  
  print $this->element("pageTitleAndButtons", $params);

  // Search Block
  if(!empty($vv_search_fields)) {
    print $this->element('search', array('vv_search_fields' => $vv_search_fields));
  }
?>
<div class="table-container">
  <table id="vetting_steps">
    <thead>
      <tr>
        <th><?php print $this->Paginator->sort('id', _txt('fd.id.seq')); ?></th>
        <th><?php print $this->Paginator->sort('CoPerson.PrimaryName.family', _txt('fd.name')); ?></th>
        <th><?php print $this->Paginator->sort('status', _txt('fd.status')); ?></th>
        <th><?php print $this->Paginator->sort('created', _txt('fd.created')); ?></th>
        <th><?php print _txt('fd.actions'); ?></th>
      </tr>
    </thead>

    <tbody>
      <?php $i = 0; ?>
      <?php foreach ($vetting_requests as $c): ?>
      <tr class="line<?php print ($i % 2)+1; ?>">
        <td>
          <?php print $this->Html->link(
            $c['VettingRequest']['id'],
            array(
              'controller' => 'vetting_requests',
              'action'     => 'view',
              $c['VettingRequest']['id']
            ));
          ?>
        </td>
        <td>
          <?php print $this->Html->link(
            generateCn($c['CoPerson']['PrimaryName']),
            array(
              'controller' => 'co_people',
              'action'     => 'canvas',
              $c['CoPerson']['id']
            ));
          ?>
        </td>
        <td>
          <?php print _txt('en.status.vet', null, $c['VettingRequest']['status']); ?>
        </td>
        <td>
          <?php print $this->Time->niceShort($c['VettingRequest']['created'], $vv_tz); ?>
        </td>
        <td>
          <?php
            if($permissions['view']) {
              print $this->Html->link(
                _txt('op.view'),
                array(
                  'controller' => 'vetting_requests',
                  'action'     => 'view',
                  $c['VettingRequest']['id']
                ),
                array(
                  'class' => 'viewbutton'
                )
              );
            }
            
            if(in_array($c['VettingRequest']['status'],
                        array(VettingStatusEnum::PendingManual,
                              VettingStatusEnum::PendingResult,
                              VettingStatusEnum::Requested))) {
              if($permissions['cancel']) {
                print '<button type="button" class="deletebutton" title="' . _txt('op.delete')
                  . '" onclick="javascript:js_confirm_generic(\''
                  . _txt('js.cancel.job') . '\',\''    // dialog body text
                  . $this->Html->url(              // dialog confirm URL
                    array(
                      'controller' => 'vetting_requests',
                      'action' => 'cancel',
                      $c['VettingRequest']['id']
                    )
                  ) . '\',\''
                  . _txt('op.remove') . '\',\''    // dialog confirm button
                  . _txt('op.cancel') . '\',\''    // dialog cancel button
                  . _txt('op.remove') . '\',[\''   // dialog title
                  . filter_var(_jtxt($c['VettingRequest']['id']),FILTER_SANITIZE_STRING)  // dialog body text replacement strings
                  . '\']);">'
                  . _txt('op.cancel')
                  . '</button>';
              }
            }
          ?>
        </td>
      </tr>
      <?php $i++; ?>
      <?php endforeach; ?>
    </tbody>
  </table>
</div>

<?php print $this->element("pagination");