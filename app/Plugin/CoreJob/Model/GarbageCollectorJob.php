...
class GarbageCollectorJob extends CoJobBackend {
  ...
  public function execute($coId, $CoJob, $params) {
    ...
    try {
      ...
      // Send notification
      $this->notify($coId, $CoJob, $params, $comment);
    }
    catch(Exception $e) {
   ...
   * @since  COmanage Registry v4.0.0
   */
  protected function notify($coId, $CoJob, $params, $comment = "", $delay=5) {
    // Load the class
    ...
    foreach($admin_list as $identifier => $copid) {
      ...
      $ids = $CoNotification->register($copid,                                 // $subjectCoPersonId
                                       ...
                                       $comment,                               // comment
                                       array(
                                         'controller' => $params['weblocation'] . '/co_jobs',
                                         'action' => 'view',
                                         'id' => $CoJob->id
                                       ),                                      // $source
                                       false,                                  // $resolve
  ...
  public function parameterFormat() {
    $params = array(
      'weblocation' => array(
        'help'     => _txt('pl.garbagecollectorjob.arg.weblocation'),
        'type'     => 'string',
        'required' => true
      ),
      'object_type' => array(
