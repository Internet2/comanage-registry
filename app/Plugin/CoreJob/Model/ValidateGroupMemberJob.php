...
class ValidateGroupMemberJob extends CoJobBackend {
  /**
  ...
  public function execute($coId, $CoJob, $params) {
    ...
    $w = $CoSetting->getGroupValiditySyncWindow($coId);

    if($w <= 0) {
      $CoJob->finish($CoJob->id, _txt('sh.job.gv.disabled'), JobStatusEnum::Failed);
    }

    try {
      $CoGroupMember->reprovisionByValidity($coId, $w, $CoJob->id);
      
      $CoJob->finish($CoJob->id, _txt('pl.groupvalidityjob.done'));
    }
    catch(Exception $e) {
