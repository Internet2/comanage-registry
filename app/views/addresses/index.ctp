...
<table id="addresses" class="ui-widget">
  ...
  <tbody>
    ...
    <tr class="line<?php print ($i % 2)+1; ?>">
      ...
      <td>
        <?php
          if(!empty($a['Address']['org_person_id']))
          {
      ...
      <td>
        <?php
          if(!empty($a['Address']['co_person_id']))
          {
            // Generally, someone who has view permission on a telephone number can also see a person
            if($permissions['view'])
      ...
      <td>    
        <?php
          ...
          if($permissions['delete'])
            echo '<button class="deletebutton" title="' . _txt('op.delete') . '" onclick="javascript:js_confirm_delete(\'' . Sanitize::html($a['Address']['line1']) . '\', \'' . $html->url(array('controller' => 'addresses', 'action' => 'delete', $a['Address']['id'])) . '\')";>' . _txt('op.delete') . '</button>';
        ?>
        <?php ; ?>
      </td>
    </tr>
