...
<table id="email_addresses" class="ui-widget">
  ...
  <tbody>
    ...
    <tr class="line<?php print ($i % 2)+1; ?>">
      ...
      <td>
        <?php
          if(!empty($e['EmailAddress']['org_person_id']))
          {
            // Generally, someone who has view permission on a telephone number can also see a person
            if($permissions['view'])
              echo $html->link(generateCn($e['OrgPerson']['Name']),
                               array('controller' => 'org_people', 'action' => 'view', $e['OrgPerson']['id'])) . "\n";
          }
        ?>
      </td>
      <td>
        <?php
          if(!empty($e['EmailAddress']['co_person_id']))
          {
            // Generally, someone who has view permission on a telephone number can also see a person
            if($permissions['view'])
      ...
      <td>    
        <?php
          ...
          if($permissions['delete'])
            echo '<button class="deletebutton" title="' . _txt('op.delete') . '" onclick="javascript:js_confirm_delete(\'' . Sanitize::html($e['EmailAddress']['mail']) . '\', \'' . $html->url(array('controller' => 'email_addresses', 'action' => 'delete', $e['EmailAddress']['id'])) . '\')";>' . _txt('op.delete') . '</button>';
        ?>
        <?php ; ?>
      </td>
    </tr>
